...
def fill_pe(ticker, tpl):
    ...
    figure = plot_vs_figure(
        title='Stock vs Industry TTM P/E Ratio',
    ...
    tpl = tpl.replace("{{ttm_pe_title}}", "<h3>Stock TTM P/E Ratio</h3>")
    ...
    ttm_pe_table = pd.DataFrame([
        ...
        {'Metrics': 'Historical average', 'Value': f"{mean:.2f}"},
        {'Metrics': 'Reasonable Range(mean-1σ ~ mean +1σ)', 'Value': f"{mean - std:.2f} ~ {mean + std:.2f}"}])
