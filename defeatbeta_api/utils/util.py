...
def load_transcripts_summary_prompt_temp() -> str:
    text = files("defeatbeta_api.data.template").joinpath('transcripts_key_financial_data_prompt.md').read_text(encoding="utf-8")
    return text

def load_transcripts_summary_tools_def() -> Dict[str, Any]:
    text = files("defeatbeta_api.data.template").joinpath('transcripts_key_financial_data_tools.json').read_text(encoding="utf-8")
    data = json.loads(text)
    return data
