...
| Pretax Income (`pretax_income_usd`)              | Financial statements (`pretax_income`)    | Reported pretax income, converted into USD using the spot FX rate at the fiscal quarter end; aligned to market capitalization `report_date` using `merge_asof` (looking back to the most recent available value)        | USD   |
