---
name: defeatbeta-3-statement-model
...
## DefeatBeta MCP Data Source

For public-company historical financial data, use the `defeatbeta-api` MCP server as the default data source. Prefer these tools when the model template requires public-company data:

| Need | DefeatBeta MCP tool |
|---|---|
| Company profile and sector | `get_stock_profile` |
| Annual income statement | `get_stock_annual_income_statement` |
| Quarterly income statement | `get_stock_quarterly_income_statement` |
| Annual balance sheet | `get_stock_annual_balance_sheet` |
| Quarterly balance sheet | `get_stock_quarterly_balance_sheet` |
| Annual cash flow statement | `get_stock_annual_cash_flow` |
| Quarterly cash flow statement | `get_stock_quarterly_cash_flow` |
| SEC filing list | `get_stock_sec_filings` |

Preserve the original model-building workflow below. DefeatBeta changes the data source, not the modeling standards.
...
For DefeatBeta, first use `get_stock_sec_filings` to locate available filings when filing-level source checks are required.
